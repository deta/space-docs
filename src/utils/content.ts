...
export const generateTitle = (path: string) =>
  path.split(/[\s-]/g).map(pascalCase).join(' ')
  ...
  const pages: Page[] = files.map(post => {
    ...
    const parts =
      post?.url
        ...
        .slice(2, -1)
        .filter(v => v) || []
    const locale = locales.includes(parts[0])
      ? parts.shift() || defaultLocale
      : defaultLocale
      ...
      if (existingIdx !== -1) {
        if (result[existingIdx]?.children?.length) {
          ...
          if (existingChildIndex !== -1) {
            if (
              (result[existingIdx].children[existingChildIndex] as any)
                .children === undefined
            ) {
              console.log(
                `[parsing]: conflicting page title with section title: "${sections[1]}"`
              )
              return
            } else {
              ;(
                result[existingIdx].children[existingChildIndex] as SectionItem
              ).children.push(pageItem)
            }
