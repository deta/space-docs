...
We've now changed this so that Python Micros are run using the same mechanism as the other Micros.
You simply specify a `run` command that starts a HTTP server on the port specified by the `PORT` environment variable.

Here's an example Spacefile for a Python Micro using the [FastAPI](https://fastapi.tiangolo.com/) framework:
