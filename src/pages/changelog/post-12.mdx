---
title: "CLI, Teletype and Rust Improvements"
date: "30 Mar 2023"
layout: "@changelogs"
description: "Improved .spaceignore, CLI bug fixes, more Teletype actions and better Rust support."
---

import DetaHeart from '@/components/DetaHeart.astro'
import Image from '@/components/Image.astro'

<Image src="/changelog_assets/teletype-project-actions.png" alt="Searching for Projects" width="90%" background />

This week we have been working on improving the CLI by expanding the default the `.spaceignore` file, improved the validation of the `Spacefile` using JSON Schema, added more Teletype actions like searching for collections, viewing the builds and releases of a project and even searching through Discovery and added better Rust support.

## Rust support

You can now deploy Rust apps to Space! We have added support for Rust apps in the Space runtime. Rust support is still in alpha and requires a bit of setup to get it working:

```bash

```

You can find the full instructions in our [docs](http://localhost:3001/docs/en/quickstart-guides/rust).

## New default `.spaceignore` patterns

Up until now the Space CLI only ignore a few directories (`node_modules` and `__pycache`) by default when uploading your app's source code during `space push`. For most apps you'd have to create your own `.spaceignore` file if you wanted common hidden files, cache directories or build outputs to be ignored. This led to a lot of confusion and frustration when trying to deploy apps that had a lot of files that were not needed for the app to run.
...
```bash
https://schema.deta.space/spacefile.v0.schema.json
```

## New Teletype Actions

<Image src="/changelog_assets/teletype-new-actions.png" subtext="New Teletype Actions" width="75%" background />
...
<Image src="/changelog_assets/teletype-list-builds.png" subtext="Listing Project Builds" width="75%" background />
