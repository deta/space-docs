...
List items that match a [query](/docs/en/build/reference/deta-base/queries).
...
| JSON Payload    | Required | Type     | Description                                    |
|-----------------|----------|----------|------------------------------------------------|
| `query`         | No       | `list`   | list of a [query](/docs/en/build/reference/deta-base/queries)|
...
| `sort`          | No       | `string` | sort order based on item keys, `asc` or `desc`, default 'asc'|
  ...
  <Fragment slot="response">
    ...
    Bad requests occur in the following cases:
    - if an unsupported query is made on the `key`
