...
import LangTabsExtended from "@/components/Tabs/LangTabsExtended.astro";
...
You can use the [Base SDK](./sdk/base) to [`put`](./sdk/base#put), [`put_many`](./sdk/base#put_many) or [`insert`](./sdk/base#insert) items with an expiration timestamp (or the [HTTP API](./http-api/base) directly).
...
You can use the [Base SDK](./sdk.md) to [`update`](./sdk#update) the expiration timestamp (or the [HTTP API](./http-api/about.md) directly).
