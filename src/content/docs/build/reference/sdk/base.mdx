...
Returns an awaitable `dict` with the item's data.
If `key` is not a non-empty string, raises a `ValueError`.
If the operation did not complete successfully, raises an `Exception`.
...
If the item is found, returns an awaitable `dict` with the item's data.
Otherwise, returns `None`.
If `key` is not a non-empty string, raises a `ValueError`.
...
Returns an awaitable instance of [`FetchResponse`](#fetchresponse).
