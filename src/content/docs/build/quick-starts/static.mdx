...
However, if you're using a different framework like [Astro](https://astro.build/), [Solid](https://www.solidjs.com/) or are building static HTML apps without a framework or static site generator, you can still run it on Space by configuring the [Spacefile](/docs/en/build/fundamentals/the-space-runtime/about#the-spacefile).
...
Here, the [`engine`](/docs/en/build/fundamentals/the-space-runtime/about#the-spacefile#engine) parameter specifies the type of engine used for the app, which is `static` in this case. The [`commands`](/docs/en/build/fundamentals/the-space-runtime/about#the-spacefile#commands) parameter is a list of commands that will be executed to build the app. The [`serve`](/docs/en/build/fundamentals/the-space-runtime/about#the-spacefile#serve) parameter specifies the directory that will be served as the root directory for the app, and in this case, the `dist` directory is used.
