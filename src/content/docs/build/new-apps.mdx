...
If [`space new`](/docs/en/build/new-apps#creating-a-project-with-builder) doesn't bootstrap your Micro when creating a project locally, you can add it by writing to the [`Spacefile`](/docs/en/build/fundamentals/the-space-runtime#the-spacefile).
...
- provide Space's [build pipeline](/docs/en/build/fundamentals/development/pushing#the-space-build-pipeline) with build commands for your Micro via `build`
...
To get started writing data or files to Base or Drive, you can use the [Deta SDKs](/docs/en/build/reference/sdk) or [Deta HTTP APIs](/docs/en/build/reference/http-api). Authentication is fully managed by Space if you use the SDKs and the [`space dev` command for local development](/docs/en/build/fundamentals/development/local-development).
...
[Read more about publishing on Space](/docs/en/publish).
